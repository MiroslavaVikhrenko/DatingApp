...
public class MessagesController(IMessageRepository messageRepository, IMemberRepository memberRepository) : BaseApiController
{
    ...
    {
        messageParams.MemeberId = User.GetMemberId();
    ...
    [HttpDelete("{id}")] //message id => not technically deleting for sure 
    // as for deletion both recipient and sender have to delete message
    public async Task<ActionResult> DeleteMessage(string id)
    {
        ...
        if (message is { SenderDeleted: true, RecipientDeleted: true }) // pattern match
        {
